...
const IOS_CONFIG = {
  ...
  GAME_TITLE_SIZE: 'text-base',        
  GAME_TITLE_SIZE_LARGE: 'text-lg',   

  RENTAL_INFO_TEXT_SIZE: 'text-sm',    
  RENTAL_INFO_TEXT_SIZE_LARGE: 'text-base', 
...
const FixedHeader = memo(({ 
  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 w-full z-40 shadow-sm flex flex-col justify-end transition-colors ${
    ...
    >
      <div className="w-full h-14 px-4 pb-2 flex justify-between items-center">
        <div>
          <div className="flex items-center gap-2 mb-0.5">
            <img 
              src="/header_logo.png" 
              alt="kakao board games" 
              className={`w-auto object-contain drop-shadow-sm ${isIosDevice ? IOS_CONFIG.HEADER_LOGO_HEIGHT : 'h-8'}`}
...
const FixedBottomNav = memo(({ 
  ...
  return (
    <nav 
      className={`fixed bottom-0 left-0 right-0 w-full z-40 shadow-lg transition-colors ${
    ...
    >
      <div className="flex justify-around items-center h-14 px-2 relative z-10">
        ...
        <button onClick={() => handleTabChange('sites')} className={`flex flex-col items-center font-bold ${isIosDevice ? IOS_CONFIG.NAV_TEXT_SIZE : 'text-[10px]'} ${activeTab === 'sites' ? isDarkMode ? 'text-white' : 'text-slate-900' : 'text-slate-400'}`}>
          ...
          <span className="mt-0.5">사이트</span>
        </button>
        {isAdmin && (
          <button onClick={() => handleTabChange('admin')} className={`flex flex-col items-center font-bold relative ${isIosDevice ? IOS_CONFIG.NAV_TEXT_SIZE : 'text-[10px]'} ${activeTab === 'admin' ? 'text-sky-500' : 'text-slate-400'}`}>
            ...
            <span className="mt-0.5">관리자</span>
          </button>
        )}
      </div>
    </nav>
...
export default function App() {
  ...
  return (
    <div className={`min-h-screen w-full relative transition-colors ${isDarkMode ? 'bg-[#0f172a] text-slate-100' : 'bg-white text-slate-900'}`}>
                    ...
                    <div key={game.gameId} className={`w-full border rounded-2xl p-3.5 flex flex-col justify-between gap-2.5 shadow-sm transition ${
                          ...
                          <div>
                            <div className="flex justify-between items-start gap-1">
                              <h3 className={`font-bold leading-snug break-keep ${
                                ...
                                <span className={`text-slate-400 font-mono font-normal whitespace-nowrap ml-1 ${
                                  isIosDevice ? 'text-xs' : 'text-[11px]'
                                }`}>({game.releaseYear}년)</span>
                              </h3>
                              <span className={`text-slate-400 font-mono flex-shrink-0 ml-1 ${
                                isIosDevice ? 'text-[11px]' : 'text-[10px]'
                              }`}>{game.gameId}</span>
                            </div>
                      ...
                      <div className="flex justify-between items-center gap-2 pt-1 w-full">
                        ...
                        <div 
                          ...
                          className={`cursor-pointer group flex items-center gap-1.5 font-bold text-slate-400 hover:text-rose-500 transition ${
                        ...
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                          ...
                          {isAvailable ? (
                            <button
                              ...
                              className={`w-auto px-3.5 py-1.5 rounded-xl font-bold flex items-center justify-center gap-1 transition ${
                                isIosDevice ? 'text-sm' : 'text-xs'
                              } ${
                                isSelectedInCart
          ...
          <div className="space-y-4 mt-0.5 w-full">
            <div className={`grid grid-cols-5 gap-1 p-1 rounded-xl font-bold w-full ${isDarkMode ? 'bg-slate-800' : 'bg-slate-100'}`}>
              <button
                ...
                className={`py-2.5 px-1 rounded-lg transition text-center whitespace-nowrap ${
                  isIosDevice ? 'text-xs font-bold' : isLargeFont ? 'text-[11px]' : 'text-[10px] font-extrabold'
                ...
                <div className={`flex p-1 rounded-xl font-bold w-full ${isDarkMode ? 'bg-slate-800' : 'bg-slate-100'}`}>
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg transition ${
                      isIosDevice ? 'text-sm font-bold' : 'text-xs'
                    } ${
                      ...
                      return (
                        <div key={rental.rentalId} className={`w-full p-3.5 rounded-2xl border ${
                          ...
                          <div className={`mt-3 pt-2 border-t flex justify-between font-medium ${
                            ...
                            <span>반납예정일: <strong className={isOverdue ? 'text-rose-600 font-bold' : isDarkMode ? 'text-slate-100 font-bold' : 'text-slate-900 font-bold'}>{rental.endDate}</strong></span>
                          </div>
                        </div>
                      );
                    })}
                ...
                <div className="space-y-2.5 w-full">
                  {sites.map((s: BoardSite) => (
                    <div key={s.siteId} className={`w-full border p-3.5 rounded-2xl shadow-sm space-y-2 ${
                      ...
                      <p className="text-slate-400 text-xs truncate font-mono">{s.url}</p>
                      <p className={`text-sm ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>{s.description}</p>
                    </div>
                  ))}
                </div>
              </div>
                    ...
                    return (
                      <div key={user.userId} className={`w-full border p-3.5 rounded-2xl space-y-2 shadow-sm ${
                        ...
                        <div className="flex justify-between items-start">
                          <div>
                            <div className="flex items-center gap-1.5">
                              <h3 className={`font-bold text-sm ${isWithdrawn ? 'text-slate-400 line-through' : isDarkMode ? 'text-slate-100' : 'text-slate-900'}`}>{user.userId}</h3>
      ...
      {/* 장바구니 플로팅 버튼 */}
      {activeTab === 'games' && (
        <div className="fixed bottom-[calc(env(safe-area-inset-bottom,0px)+68px)] right-4 pointer-events-none z-30 transition-all">
          <button
      ...
      <div className={`fixed inset-0 z-50 transition-all duration-300 ${
        ...
        <div className={`absolute top-0 right-0 h-full w-1/3 min-w-[200px] max-w-xs flex flex-col shadow-2xl transition-transform duration-300 ease-out ${
          isSettingsOpen ? 'translate-x-0' : 'translate-x-full'
        } ${isDarkMode ? 'bg-slate-900 text-slate-100' : 'bg-white text-slate-900'}`}>
          <div 
            style={{ paddingTop: 'calc(env(safe-area-inset-top, 0px) + 16px)' }}
            className="p-4 flex justify-between items-center font-bold text-base bg-[#FEE500] text-slate-900"
          >
      ...
      <div className={`fixed inset-0 z-50 transition-all duration-300 ${
        ...
        <div className={`absolute top-0 right-0 h-full w-4/5 max-w-sm flex flex-col shadow-2xl transition-transform duration-300 ease-out ${
          ...
          <div 
            style={{ paddingTop: 'calc(env(safe-area-inset-top, 0px) + 16px)' }}
            className="p-4 bg-[#FEE500] text-slate-900 flex justify-between items-center font-bold text-base"
          >
            <span className="flex items-center gap-2">
              <Bell size={18} className="text-slate-900" /> 공지사항 목록
            </span>
          ...
          >
            <button
              ...
              className="w-full bg-slate-900 dark:bg-slate-800 text-white dark:text-slate-200 py-3 rounded-xl font-bold hover:bg-slate-800 transition text-sm"
            >
              닫기
            </button>
          </div>
                  ...
                  <div key={game.gameId} className={`p-3 rounded-2xl border flex items-center justify-between gap-3 shadow-sm ${
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      <img 
                        src={game.imageUrl} 
                        alt={game.title} 
                        className="w-11 h-11 object-cover rounded-xl bg-white border border-slate-200/50 dark:border-slate-700 flex-shrink-0"
                        onError={(e) => { (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1610890716171-6b1bb98ffd09?w=300'; }}
                      />
