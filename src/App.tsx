...
export default function App() {
  ...
  return (
    // ⭕ [하단 네비게이션 복구 레아아웃]: flex flex-col h-[#100dvh] 구조로 전체 뷰포트에서 상/하단바 노출 보장
    <div className={`h-[#100dvh] w-full flex flex-col justify-between overflow-hidden transition-colors ${isDarkMode ? 'bg-[#0f172a] text-slate-100' : 'bg-white text-slate-900'}`}>
      ...
      <main 
        ...
        className={`flex-1 w-full py-4 ${isIosDevice ? IOS_CONFIG.MAIN_PADDING_X : 'px-4'} pb-24 overflow-y-auto overscroll-y-contain transition-colors ${isDarkMode ? 'bg-[#0f172a]' : 'bg-white'} ${
          isLargeFont 
